...
from .test import cron

_MAIN_CRON_MODULE = 'django_cron.cron'
_TEST_CRON_MODULE = 'django_cron.test.cron'
...
class TestRunCrons(TransactionTestCase):
    cron_success = f'{_TEST_CRON_MODULE}.TestSuccessCronJob'
    cron_error = f'{_TEST_CRON_MODULE}.TestErrorCronJob'
    cron_5mins = f'{_TEST_CRON_MODULE}.Test5minsCronJob'
    cron_5mins_with_tolerance = f'{_TEST_CRON_MODULE}.Test5minsWithToleranceCronJob'
    cron_run_at_times = f'{_TEST_CRON_MODULE}.TestRunAtTimesCronJob'
    cron_wait_3sec = f'{_TEST_CRON_MODULE}.Wait3secCronJob'
    cron_run_on_weekend = f'{_TEST_CRON_MODULE}.RunOnWeekendCronJob'
    cron_obviously_doesnt_exist = 'ThisCronJobDoesntExost'
    cron_no_code = f'{_TEST_CRON_MODULE}.NoCodeCronJob'
    cron_failed_runs_notification = f'{_MAIN_CRON_MODULE}.FailedRunsNotificationCronJob'
    run_on_month_days = f'{_TEST_CRON_MODULE}.RunOnMonthDaysCronJob'
    run_and_remove_old_logs = f'{_TEST_CRON_MODULE}.RunEveryMinuteAndRemoveOldLogs'
    ...
    def test_failed_cron(self):
        response = self._call(self.cron_error, force=True)
        self.assertReportedFail(cron.TestErrorCronJob, response)
        self.assertEqual(CronJobLog.objects.all().count(), 1)

    def test_not_exists_cron(self):
        response = self._call(self.cron_obviously_doesnt_exist, force=True)
        self.assertIn('Make sure these are valid cron class names', response)
        self.assertIn(self.cron_obviously_doesnt_exist, response)
        self.assertEqual(CronJobLog.objects.all().count(), 0)

    @patch('django_cron.core.logger')
    def test_requires_code(self, mock_logger):
        response = self._call(self.cron_no_code, force=True)
    ...
    def test_dry_run_does_not_perform_task(self, mock_do):
        response = self._call(self.cron_success, dry_run=True)
        self.assertReportedRun(cron.TestSuccessCronJob, response)
    ...
    def test_non_dry_run_performs_task(self, mock_do):
        ...
        response = self._call(self.cron_success)
        self.assertReportedRun(cron.TestSuccessCronJob, response)
    ...
    def test_runs_every_mins(self):
        with freeze_time("2014-01-01 00:00:00"):
            response = self._call(self.cron_5mins)
        self.assertReportedRun(cron.Test5minsCronJob, response)
        self.assertEqual(CronJobLog.objects.all().count(), 1)

        with freeze_time("2014-01-01 00:04:59"):
            response = self._call(self.cron_5mins)
        self.assertReportedNoRun(cron.Test5minsCronJob, response)
        self.assertEqual(CronJobLog.objects.all().count(), 1)
    ...
    def test_runs_at_time(self):
        with freeze_time("2014-01-01 00:00:01"):
            response = self._call(self.cron_run_at_times)
        self.assertReportedRun(cron.TestRunAtTimesCronJob, response)
        self.assertEqual(CronJobLog.objects.all().count(), 1)

        with freeze_time("2014-01-01 00:04:50"):
            response = self._call(self.cron_run_at_times)
        self.assertReportedNoRun(cron.TestRunAtTimesCronJob, response)
        self.assertEqual(CronJobLog.objects.all().count(), 1)
    ...
    def test_run_on_weekend(self):
        for test_date in ("2017-06-17", "2017-06-18"):  # Saturday and Sunday
            ...
            with freeze_time(test_date):
                call_command('runcrons', self.cron_run_on_weekend)
    ...
    def test_silent_produces_no_output_success(self):
        response = self._call(self.cron_success, silent=True)
        self.assertEqual(1, CronJobLog.objects.count())
        self.assertEqual('', response)

    def test_silent_produces_no_output_no_run(self):
        with freeze_time("2014-01-01 00:00:00"):
            response = self._call(self.cron_run_at_times, silent=True)
        self.assertEqual(1, CronJobLog.objects.count())
        self.assertEqual('', response)
    ...
    def test_silent_produces_no_output_failure(self):
        response = self._call(self.cron_error, silent=True)
        self.assertEqual('', response)
    ...
    def run_cronjob_in_thread(self, logs_count):
        self._call(self.cron_wait_3sec)
        self.assertEqual(CronJobLog.objects.all().count(), logs_count + 1)
    ...
    def test_failed_runs_notification(self):
        ...
        for i in range(10):
            self._call(self.cron_error, force=True)
        self._call(self.cron_failed_runs_notification)
...
class TestCronLoop(TransactionTestCase):
    success_cron = f'{_TEST_CRON_MODULE}.TestSuccessCronJob'
